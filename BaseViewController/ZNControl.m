...
#pragma mark 数字英文加下划线 */
+(BOOL)isDigitalEnglish:(NSString *)deu
{
    NSString *deuRegex = @"^[A-Za-z0-9]+$";
    NSPredicate *deuTest = [NSPredicate predicateWithFormat:@"SELF MATCHES %@", deuRegex];
    NSLog(@"%d",[deuTest evaluateWithObject:deu]);
    return [deuTest evaluateWithObject:deu];
}
...
//    BOOL isMatch = [mobile isMatchedByRegex:phoneRegex];
//    return isMatch;
}
#pragma mark 中文判断
...
//    for (NSUInteger i = 0; i <ch.length; i++) {
...
#pragma mark 地球坐标转为火星坐标
+(NSString *)transform2Mars:(CGFloat)wgLat  withWgLon:(CGFloat)wgLon
{
    //坐标转换用的数据
    CGFloat   pi = 3.14159265358979324;
    CGFloat  a = 6378245.0;
    CGFloat  ee = 0.00669342162296594323;
//    double  x_pi = 3.14159265358979324 * 3000.0 / 180.0;
    if ([ZNControl outOfChina:wgLat withLon:wgLon] == YES)
    {
//        self.latitude = [NSString stringWithFormat:@"%lf",wgLat];
//        self.longitude = [NSString stringWithFormat:@"%lf",wgLon];
        return [NSString stringWithFormat:@"%lf:%lf",wgLat,wgLon];
    }
    CGFloat dLat = [ZNControl transformLat:wgLon- 105.0 withY:wgLat - 35.0];
    CGFloat dLon =[ZNControl transformLon:wgLon- 105.0 withY:wgLat - 35.0];
    CGFloat radLat = wgLat / 180.0 * pi;
    CGFloat magic = sin(radLat);
    magic = 1 - ee * magic * magic;
    CGFloat sqrtMagic = sqrt(magic);
    dLat = (dLat * 180.0) / ((a * (1 - ee)) / (magic * sqrtMagic) * pi);
    dLon = (dLon * 180.0) / (a / sqrtMagic * cos(radLat) * pi);
//    self.latitude = [NSString stringWithFormat:@"%lf",wgLat + dLat];
//    self.longitude = [NSString stringWithFormat:@"%lf",wgLon + dLon];
    return [NSString stringWithFormat:@"%lf:%lf",wgLat + dLat,wgLon + dLon];
}
+(BOOL)outOfChina:(CGFloat)lat withLon:(CGFloat)lon
{
...
+(CGFloat)transformLat:(CGFloat)x withY:(CGFloat)y
{
    //坐标转换用的数据
    CGFloat   pi = 3.14159265358979324;
//    double  a = 6378245.0;
...
//    double  x_pi = 3.14159265358979324 * 3000.0 / 180.0;
    CGFloat ret = -100.0 + 2.0 * x + 3.0 * y + 0.2 * y * y + 0.1 * x * y + 0.2 * sqrt(abs(x));
    ret += (20.0 * sin(6.0 * x * pi) + 20.0 * sin(2.0 * x * pi)) * 2.0 / 3.0;
    ret += (20.0 * sin(y * pi) + 40.0 * sin(y / 3.0 * pi)) * 2.0 / 3.0;
    ret += (160.0 * sin(y / 12.0 * pi) + 320 * sin(y * pi / 30.0)) * 2.0 / 3.0;
    return ret;
}
+(CGFloat)transformLon:(CGFloat)x withY:(CGFloat)y
{
    //坐标转换用的数据
    CGFloat   pi = 3.14159265358979324;
//    double  a = 6378245.0;
//    double  ee = 0.00669342162296594323;
//    double  x_pi = 3.14159265358979324 * 3000.0 / 180.0;
    CGFloat ret = 300.0 + x + 2.0 * y + 0.1 * x * x + 0.1 * x * y + 0.1 * sqrt(abs(x));
    ret += (20.0 * sin(6.0 * x * pi) + 20.0 * sin(2.0 * x * pi)) * 2.0 / 3.0;
...
#pragma mark 等比放大 320*568 为基准  等比放大 所小  所有控件按这个写
+(CGRect)CGRectMake:(CGRect)rect{
    CGFloat x = VIEWWIDTH/320*(rect.origin.x);
    CGFloat y = VIEWHEIGHT/568*rect.origin.y;
    CGFloat width = VIEWWIDTH/320*rect.size.width;
    CGFloat height = VIEWHEIGHT/568*rect.size.height;
