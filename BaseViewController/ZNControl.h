...
#define VIEWWIDTH  [[UIScreen mainScreen] bounds].size.width   //屏幕的宽度
#define VIEWHEIGHT [[UIScreen mainScreen] bounds].size.height  //屏幕的高度
#define ADAPTATIONHEIGHT(h) (h)/568.00*VIEWHEIGHT //适配高度 适配6 6p
#define ADAPTATIONWIDTH(w) (w)/320.00*VIEWWIDTH //适配宽度 适配6 6p
#define ZNCGRECGMAKE(x,y,w,h) CGRectMake(VIEWWIDTH/320.00*(x), VIEWHEIGHT/568.00*(y), VIEWWIDTH/320.00*(w), VIEWHEIGHT/568.00*(h))//适配frame适配6 6p
#define ZNColor(r, g, b) [UIColor colorWithRed:(r)/255.0 green:(g)/255.0 blue:(b)/255.0 alpha:1.0]// 颜色
#define ZNRandomColor ZNColor(arc4random_uniform(256), arc4random_uniform(256), arc4random_uniform(256))// 随机色
#define VIEWSTATUSHEIGHT [ZCControl isIOS7]==64?20:0  //适配要减去的状态条高度
#define IOS7   [[UIDevice currentDevice]systemVersion].floatValue>=7.0
#define IOS8   [[UIDevice currentDevice]systemVersion].floatValue>=8.0


@interface ZNControl : NSObject
#pragma mark --判断设备型号
