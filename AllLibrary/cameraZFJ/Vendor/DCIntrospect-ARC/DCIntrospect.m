...
- (void)moveBackInViewHierarchy {
    ...
    [self selectView:[self.currentViewHistory objectAtQYQIndex:indexOfCurrentView - 1]];
}
...
- (void)moveDownToFirstSubview {
    ...
    if (self.currentView.subviews.count>0) {
        [self selectView:[self.currentView.subviews objectAtQYQIndex:0]];
    } else{
...
- (void)moveToNextSiblingView {
    ...
    if (currentViewsIndex==NSNotFound) {
        ...
        [self selectView:[self.currentView.superview.subviews objectAtQYQIndex:currentViewsIndex + 1]];
    } else{
...
- (UIWindow *)mainWindow
{
	...
	return [windows objectAtQYQIndex:0];
}
